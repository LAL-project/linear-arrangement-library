...
public:
	...
	struct frequencies {
		...
		{
			return counter_equal == f.counter_equal and
...
public:
	 ...
	 */
	[[nodiscard]] std::pair<const T&, frequencies>
	get_largest_value() const noexcept
	{
#if defined DEBUG
	 ...
	 */
	[[nodiscard]] std::pair<const T&, frequencies>
	get_smallest_value() const noexcept
	{
#if defined DEBUG
		assert(m_root != nullptr);
#endif
	...
	void join_sorted_all_greater(U&& v) noexcept
	{
		...
		if (v.size() == 1) {
			m_root = [&]()
			{
				if constexpr (rvalue) {
					return insert(
						nullptr, m_root, std::forward<T>(v[0]), dummy
					);
				}
				else {
...
private:
	...
	struct tree_node {
		...
		void update_height_and_balance_factor() noexcept
		{
			...
			const int64_t rh =
				(right != nullptr ? to_int64(right->height) : -1);
		...
		{
			tree_counter = node_counter + left_counter() + right_counter();
		}
...
private:
	 ...
	 * @pre Node @e n has a right subtree.
	 */
	[[nodiscard]] tree_node *right_right_case(tree_node * const n
	) const noexcept
	{
	...
	[[nodiscard]] tree_node *balance(tree_node * const n) const noexcept
	{
		...
		if (std::abs(n->balance_factor) <= 1) {
			return n;
		}
		return (
			n->balance_factor == -2
				? (n->left->balance_factor <= 0 ? left_left_case(n)
												: left_right_case(n))
				: (n->right->balance_factor >= 0 ? right_right_case(n)
												 : right_left_case(n))
		);
	}
	...
	template <typename U>
	[[nodiscard]] tree_node *
	insert(tree_node *p, tree_node *n, U&& x, frequencies& freqs) const noexcept
	{
	...
	template <bool use_counter_to_remove>
	[[nodiscard]] tree_node *
	remove(tree_node *n, const T& x, frequencies& freqs) const noexcept
	{
		if (n == nullptr) {
			// accumulate frequencies
	 ...
	 */
	[[nodiscard]] tree_node *
	join_taller(tree_node *T1, tree_node *T2) const noexcept
	{
#if defined DEBUG
		assert(T1 != nullptr);
		assert(T2 != nullptr);
	 ...
	 */
	[[nodiscard]] tree_node *
	join_shorter(tree_node *T1, tree_node *T2) const noexcept
	{
#if defined DEBUG
		assert(T1 != nullptr);
		assert(T2 != nullptr);
	...
	) const noexcept
	{
		...
		n->left =
			(l + 1 > m ? nullptr
					   : _make_tree(std::forward<U>(v), l, m - 1, n, 'l'));
		n->right =
			(m + 1 > r ? nullptr
					   : _make_tree(std::forward<U>(v), m + 1, r, n, 'r'));
...
private:
	...
	[[nodiscard]] std::size_t exhaustive_occurrences(tree_node *n
	) const noexcept
	{
		if (n == nullptr) {
			return 0;
		}
		return n->node_counter + exhaustive_occurrences(n->right) +
	...
	[[nodiscard]] bool check_relations(tree_node *n) const noexcept
	{
		...
		if (not smaller_left or not greater_right) {
			return false;
		}
	...
	[[nodiscard]] bool sanity_check(tree_node *n) const noexcept
	{
		...
		if (not check_relations(n)) {
			std::cerr << "Elements incorrectly placed in the tree.\n"
					  << "    n->key= " << n->key << '\n';
			return false;
		}

		if (std::abs(n->balance_factor) >= 2) {
			std::cerr << "Incorrect balance factor.\n"
					  << "    n->key= " << n->key << '\n'
		...
		// OCCURRENCES
		{
			...
			if (my_occurrences != n->tree_counter) {
				std::cerr << "Incorrect occurrencess.\n"
						  << "    n->key= " << n->key << '\n'
		...
		if (n->left != nullptr) {
			if (n->left->key > n->key) {
				std::cerr << "Keys do not satisfy the order requirement.\n"
						  << "    n->key= " << n->key << '\n'
	...
	void print_tree(tree_node *n, const std::string& tab) const noexcept
	{
		...
		std::cout << n->key << ", nc= " << n->node_counter << ", s= " << n->side
				  << ", ls= " << n->left_size() << ", rs= " << n->right_size()
				  << ", h= " << n->height << ", ts= " << n->tree_size
				  << ", tc= " << n->tree_counter
				  << ", bf= " << n->balance_factor << '\n';
