...
propagation_result
AEF_BnB::propagate_LV__bridge__check_lowest_can_be_predicted
(const std::size_t path_idx, const LV_propagation_origin origin) noexcept
{
...
void AEF_BnB::propagate_LV__bridge__from_hub__h2
...
void AEF_BnB::propagate_LV__bridge__from_hub__h1
(const std::size_t path_idx) noexcept
{
	const auto& path = m_paths_in_tree[path_idx];
	const node w = path.get_lowest_lexicographic();
...
propagation_result
AEF_BnB::propagate_LV__bridge__from_hub
(const node h, const std::size_t path_idx) noexcept
{
...
#if defined __LAL_DEBUG_DMax_Unc_BnB
	std::cout << tab() << "Propagate through a bridge from one of the hubs...\n";
	std::cout << tab() << "    hub= " << h << '\n';
	std::cout << tab() << "    is h1? " << std::boolalpha << (h == path.get_h1()) << '\n';
	std::cout << tab() << "    is h2? " << std::boolalpha << (h == path.get_h2()) << '\n';
#endif
	...
	return propagate_LV__bridge__check_lowest_can_be_predicted(path_idx, origin);
}
...
void AEF_BnB::propagate_LV__bridge__from_lowest__level_0
(const node u) noexcept
{
	...
	if (path.get_num_nodes() == 3) { return; }
...
propagation_result
AEF_BnB::propagate_LV__bridge__from_lowest__level_pm2
(const node u) noexcept
{
	...
	if (v != path.get_h1() and m_predicted_LV[v] != 2*sign) {
		...
		std::cout << tab() << "    Existing prediction= " << m_predicted_LV[v] << '\n';
...
propagation_result
AEF_BnB::propagate_LV__bridge__from_internal
(const node u) noexcept
{
	const std::size_t path_idx = m_node_to_path_idx[u];
	const auto& path = m_paths_in_tree[path_idx];
	...
	const LV_propagation_origin orig = [&]() {
		return
			pu < pw ?
				LV_propagation_origin::bridge_internal_left :
