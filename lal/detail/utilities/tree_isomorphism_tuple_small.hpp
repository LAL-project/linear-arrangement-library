 ...
 */
inline void calculate_ids_small(
 ...
 */
[[nodiscard]] inline bool are_rooted_trees_isomorphic_tuple_small(
...
) noexcept
{
	...
	while (not stop) {
		...
		if (h > 0) {
			isomorphism::calculate_ids_small(t1, S1, tuple_ids_1);
			isomorphism::calculate_ids_small(t2, S2, tuple_ids_2);
...
) noexcept
{
	...
	while (not stop) {
		...
		// Step 3: Construct the ids of the current tuples.
		if (h > 0) {
			isomorphism::calculate_ids_small(parents_1, S1, tuple_ids_1);
			isomorphism::calculate_ids_small(parents_2, S2, tuple_ids_2);
