...
public:
	 ...
	 */
	[[nodiscard]] const neighbourhood& get_out_neighbors(const node u
	) const noexcept
	{
#if defined DEBUG
		assert(has_node(u));
	 ...
	 */
	[[nodiscard]] const neighbourhood& get_in_neighbors(const node u
	) const noexcept
	{
#if defined DEBUG
		assert(has_node(u));
	...
	[[nodiscard]] std::vector<directed_graph>
