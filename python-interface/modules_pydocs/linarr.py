...
The contents of a ``linear_arrangement`` object are as follows: if :math:`\pi` is a linear arrangement then the `u`-th position of :math:`\pi` contains the position of that node in the arrangement. Formally, :math:`\pi[u] = p` if, and only if, node `u` is at position `p` in the linear arrangement.
