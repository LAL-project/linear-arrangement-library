...
This modules contains functions to calculate properties of graphs that depend on a linear arrangement. Said arrangement van be given explicitly, i.e., by constructing a ``linear_arrangement`` object, or by omitting it in the functions to let these use the labelling of the graph. For example, given a graph
...
The contents of a ``lal.linear_arrangement`` object are as follows: if :math:`\pi` is a linear arrangement then the `u`-th position of :math:`\pi` contains the position of that node in the arrangement. Formally, :math:`\pi[u] = p` if, and only if, node `u` is at position `p` in the linear arrangement.
