...
implemented uses Prüfer sequences (see [22]) and decodes them using the
algorithm in [4].  

In order to use this class, you must first provide the size $n$ of the tree
...
false. At this point, method get_tree will always construct a star tree of $n$
nodes.  
...
class will generate $n!$ arrangements for all star trees of $n$ vertices.  
...
false. At this point, method get_tree will always construct a star tree of $n$
nodes rooted at its central node.  
...
The set $Q(G)$ is defined as the pairs of edges of $G$, $E(G) \\times E(G)$,
...
The undirected graph returned connects two vertices $u,v$ if these two vertices
are connected by a directed edge ( $(u,v)$ or $(v,u)$) in this graph. In other
...
returned is an edge $(u,v)$ so that the inequality $u < v$ always holds. For
...
pre: Edge $\\{s,t\\}$ is not part of the graph.  
...
Iterator over the set $Q$ of a graph.  

This class is used to easily iterate over the elements of the set $Q$ of a
graph.  

This class iterates over the independent pairs of edges of a graph. For
undirected graphs, the edges of the pair returned are edges $(u,v)$ so that the
inequality $u < v$ always holds. For directed graphs, this is not always true,
...
If this rational value was $n/d$ then after calling this method it will be
$d/n$.  
";
...
If *relab* is true then the nodes are relabelled to numbers in $[0, n_u)$, where
$n_u$ is the number of nodes of the subtree rooted at *u*, rather than keeping
the original labelling of numbers in $[0,n)$, where *n* is the number of nodes
of the tree.  
...
Overloaded function
-------------------
* `insert_sorted(x) -> std::vector< T >::iterator`  
...
Overloaded function
-------------------
* `insert_sorted_unique(x) -> std::vector< T >::iterator`  
...
Overloaded function
-------------------
* `n_C_brute_force_list(g, pis) -> std::vector< uint32_t >`  
...
Overloaded function
-------------------
* `n_C_dynamic_programming(g, pi={}) -> uint32_t`  
...
Overloaded function
-------------------
* `n_C_dynamic_programming_list(g, pis) -> std::vector< uint32_t >`  
...
Overloaded function
-------------------
* `n_C_ladder_list(g, pis) -> std::vector< uint32_t >`  
...
Overloaded function
-------------------
* `make_arrangement_intervals(T, data) -> linear_arrangement`  

* `make_arrangement_intervals(T, root, data) -> linear_arrangement`  
";
...
Overloaded function
-------------------
* `retrieve_centroid(t, x, M, sizes_edge) -> std::pair< node, node >`  

* `retrieve_centroid(t, x) -> std::pair< node, node >`  

* `retrieve_centroid(t, M, sizes_edge) -> std::pair< node, node >`  

* `retrieve_centroid(t) -> std::pair< node, node >`  
";
...
Computes the 1-level Mean Dependency Distance $MDD$ over an ensemble of graphs.  

Given a list of graphs $L_i$ and a linear arrangement of the nodes for each of
them, computes the 1-level Mean Dependency Distance as the quotient of $D$, the
sum of all the $D_i$, where $D_i$ is the sum of the edge lengths of the $i$-th
graph, and of $M$ the sum of the number of edges of all the graphs.  

Formally, given a list of linear arrangements $\\Pi = \\{\\pi_i\\}_{i=1}^k$ and
a list of graphs $G = \\{G_i\\}_{i=1}^k$, computes $D/M$, where  

*   $D = \\sum_{i=1}^k D(G_i, \\pi_i)$ is the sum of edge lengths of every
    graph.  
*   $M = \\sum_{i=1}^k 
E(G_i)
$ is the sum of the number of edges of every
    graph.  
    ...
    * `pis` :  
        List of linear arrangements of the nodes $\\Pi = \\{\\pi_i\\}_{i=1}^k$.
...
* `Gs` :  
    List of input graphs.  
* `pis` :  
    List of linear arrangements of the nodes $\\Pi = \\{\\pi_i\\}_{i=1}^k$. When
    omitted, $\\pi_I$ is used for all graphs.  
...
Computes the 2-level Mean Dependency Distance $MDD$ over an ensemble of graphs.  

Given a list of graphs and a linear arrangement of the nodes for each of them,
computes the 2-level Mean Dependency Distance, i.e., it computes the average
Mean Dependency Distance of the graphs in the list.  

Formally, given a list of linear arrangements $\\Pi = \\{\\pi_i\\}_{i=1}^k$ and
a list of graphs $G = \\{G_i\\}_{i=1}^k$, computes $(1/k)S_{<d>}$, where
$S_{<d>} = \\sum_{i=1}^k MDD(G_i, \\pi_i)$ is the sum of the mean dependency
distances of every graph (see mean_dependency_distance_rational for details on
...
Overloaded function
-------------------
* `n_crossings(g, pi={}, A=algorithms_C::ladder) -> uint32_t`  
    ...
    * `g` :  
        Input graph.  
    * `pi` :  
        A linear arrangement of the nodes. When omitted, $\\pi_I$ is used.  
    * `A` :  
...
Overloaded function
-------------------
* `n_crossings_list(g, pis, A=algorithms_C::ladder) -> std::vector< uint32_t >`  
    ...
    * `g` :  
        Input graph.  
    * `pis` :  
        List of $k$ linear arrangements of the nodes $\\Pi =
        \\{\\pi_i\\}_{i=1}^k$.  
    * `A` :  
        Algorithm to use to compute the number of crossings. Default:
        algorithms_C::stack_based.  
...
Overloaded function
-------------------
* `approximate_C_rational(g, pi={}) -> numeric::rational`  
    ...
    * `pi` :  
    ...
    Returns:
    Approximation of the number of crossings $E_s[C_G\\;
\\;\\delta]$.  
...
* `t` :  
    Input tree.  
* `pi` :  
    Linear arrangement of the nodes. If $\\pi[u]=p$ then node *u* is placed in
...
* `sum_length_edges(g, pi={}) -> uint32_t`  
    ...
    Given a graph $G$ and a linear arrangement of its nodes $\\pi$, computes the
    sum of the length of the graph's edges in the arrangement. Formally, this
    function computes the value $D_{\\pi}(G)=\\sum_{uv\\in E(G)} 
\\pi(u) -
    \\pi(v)
$.  
...
* `mean_dependency_distance_rational(g, pi={}) -> numeric::rational`  
    ...
    it computes $\\frac{D_{\\pi}(G)}{
E(G)
}$. See function sum_length_edges for
    further details on $D_{\\pi}(G)$.  
...
Overloaded function
-------------------
* `mean_dependency_distance(g, pi={}) -> double`  
...
Overloaded function
-------------------
* `Dmin(t, a) -> std::pair< uint32_t, linear_arrangement >`  
    
    Computes the minimum value of $D$ in rooted trees.  

    Calculates the minimum value of $D$ and returns a linear arrangement
    yielding this value. Such optimal value of $D$ depends on the choice of
...
Given a graph and a permutation of its nodes, the headedness $h$ is the ratio of
right-branching edges over the total amount of edges. More precisely, it is  

$h = \\frac{r}{m}$  

where $r$ is the number of right-branching edges and $m$ is the number of edges
of the graph.  
...
* `g` :  
    Input graph.  
* `pi` :  
    Permutation of the nodes. When omitted, $\\pi_I$ is used.  
...
Computes $V_{rla}[C]$ on the given graph. This function implements the algorithm
...
Computes $V_{rla}[C]$ on the given forest. This function implements the
...
Computes $V_{rla}[C]$ on the given tree. This function computes the simplified
formula of $V_{rla}[C]$ on general graphs for the case of trees. Complexity:
time $O(n)$, space $O(n)$.  
...
* `mmt_degree_rational(g, p) -> numeric::rational`  
    ...
    Computes the $p$-th moment of degree about zero of a graph as an exact
    rational value.  

    Computes the $p$-th moment of degree about zero, $\\langle k^p \\rangle$, of
    a graph using:  

    $\\langle k^p \\rangle = \\frac{1}{n} \\sum_{i=1}^n k_i^p $.  

    where $n$ denotes the number of nodes of the graph and $k_i$ is the degree
    of vertex $i$.  
...
* `mmt_degree_rational(g, p) -> numeric::rational`  
    ...
    Computes the $p$-th moment of degree about zero of a directed graph as an
    ...
    where $n$ denotes the number of nodes of the graph and $k_i$ is the in-
    degree plus the out-degree of vertex $i$.  
...
Computes the $p$-th moment of in-degree about zero of a directed graph as an
exact rational value.  

Computes the $p$-th moment of in-degree about zero, $\\langle k_{in}^p
...
Computes the $p$-th moment of out-degree about zero of a directed graph as an
exact rational value.  

Computes the $p$-th moment of out-degree about zero, $\\langle k_{out}^p
\\rangle$, of a directed graph using:  
...
$ h = \\frac{ \\langle k^2 \\rangle - \\langle k^2 \\rangle_{linear} } {
\\langle k^2 \\rangle_{star} - \\langle k^2 \\rangle_{linear} } $,  

where $\\langle k^2 \\rangle_{star}$ and $\\langle k^2 \\rangle_{linear}$ are
...
hierarchical distance $HD_u$ of vertex $u$ is calculated as the number of edges
between the tree's root and $u$. The result of this function is the mean of
these distances: $MHD = \\frac{1}{n} \\sum_{u\\in V} HD_u$.  
...
* `size_Q_integer(g) -> numeric::integer`  
    ...
    The set $Q(G)$ of a graph $G$ is the set of pairs of independent edges. Two
...
Overloaded function
-------------------
* `get_centre(t) -> std::pair< node, node >`  
...
Overloaded function
-------------------
* `get_centroid(t) -> std::pair< node, node >`  
...
Overloaded function
-------------------
* `are_trees_isomorphic(t1, t2) -> bool`  
    
    Isomorphism test for rooted trees.  

    Decides whether the input trees are isomorphic or not. Two trees $t_1$ and
    $t_2$ (or graphs in general) are isomorphic if there exists a mapping $\\phi
    \\;:\\; V(t_1) \\longrightarrow V(t_2)$ such that  

    $ \\forall u,v\\in V(t_1) \\; (u,v)\\in E(t_1) \\longleftrightarrow
    (\\phi(u),\\phi(v))\\in E(t_2) $  

    and $\\phi(r_1)=r_2$ where $r_1$ and $r_2$ are, respectively, the roots of
    $t_1$ and $t_2$. Note that $(u,v)$ denotes a directed edge.  
...
* `are_trees_isomorphic(t1, t2) -> bool`  
    ...
    $ \\forall u,v\\in V(t_1) \\; uv\\in E(t_1) \\longleftrightarrow
    \\phi(u)\\phi(v)\\in E(t_2) $  
...
// File: 1level__impl_8hpp.xml

// File: 2level_8hpp.xml

// File: 2level__impl_8hpp.xml
